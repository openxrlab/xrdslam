...
def matrix_to_quaternion(matrix: torch.Tensor) -> torch.Tensor:
    ...
    # same (up to a sign), forall i; we pick the best-conditioned
    # one (with the largest denominator)
...
def params2rendervar(params):
    rendervar = {
        ...
        params['rgb_colors'],
        'rotations':
        F.normalize(params['unnorm_rotations']),
        'opacities':
        torch.sigmoid(params['logit_opacities']),
        'scales':
        torch.exp(torch.tile(params['log_scales'], (1, 3))),
        'means2D':
        torch.zeros_like(params['means3D'], requires_grad=True, device='cuda')
        + 0
    }
    return rendervar
...
def params2depthplussilhouette(params, w2c):
    rendervar = {
        ...
        params['means3D'],
        'colors_precomp':
        get_depth_and_silhouette(params['means3D'], w2c),
        'rotations':
        F.normalize(params['unnorm_rotations']),
        'opacities':
        torch.sigmoid(params['logit_opacities']),
...
def transformed_params2depthplussilhouette(params, w2c, transformed_pts):
    rendervar = {
        ...
        transformed_pts,
        'colors_precomp':
        get_depth_and_silhouette(transformed_pts, w2c),
        'rotations':
        F.normalize(params['unnorm_rotations']),
        'opacities':
        torch.sigmoid(params['logit_opacities']),
...
def transform_to_frame(means3D, w2c, gaussians_grad, camera_grad):
    """Function to transform Isotropic Gaussians from world frame to camera
    frame.

    Args:
        means3D: Centers and norm Rots of Gaussians in World Frame
        w2c: pose
