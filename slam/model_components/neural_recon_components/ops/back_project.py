...
def back_project(coords, origin, voxel_size, feats, KRcam):
    ...
    :param origin: origin of the partial voxel volume (voxel xyz (0, 0, 0))
    ...
    for batch in range(bs):
        ...
        nV = rs_grid.shape[-1]
        rs_grid = torch.cat(
            [rs_grid, torch.ones([n_views, 1, nV]).cuda()], dim=1)
        ...
        im_grid = torch.stack([2 * im_x / (w - 1) - 1, 2 * im_y / (h - 1) - 1],
                              dim=-1)
        mask = im_grid.abs() <= 1
        ...
        features = grid_sample(feats_batch,
                               im_grid,
                               padding_mode='zeros',
                               align_corners=True)
        ...
        im_z = im_z.sum(dim=0).unsqueeze(1) / in_scope_mask.permute(
            1, 0).contiguous()
