...
def completion_ratio(gt_points, rec_points, dist_th=0.05):
    ...
    comp_ratio = np.mean((distances < dist_th))
    return comp_ratio
...
def check_proj(points, W, H, fx, fy, cx, cy, c2w):
    ...
    K = torch.from_numpy(
        np.array([[fx, .0, cx], [.0, fy, cy], [.0, .0,
                                               1.0]]).reshape(3, 3)).cuda()
    ones = torch.ones_like(points[:, 0]).reshape(-1, 1).cuda()
    homo_points = torch.cat([points, ones],
                            dim=1).reshape(-1, 4, 1).cuda().float()  # (N, 4)
    ...
    edge = 0
    mask = (0 <= -z[:, 0, 0].cpu().numpy()) & (uv[:, 0] < W - edge) & (
        uv[:, 0] > edge) & (uv[:, 1] < H - edge) & (uv[:, 1] > edge)
    return mask.sum() > 0


def calc_3d_metric(rec_meshfile,
                   gt_meshfile,
                   align=True,
    ...
    if transform_matrix is not None:
        ...
        o3d_rec_pc = o3d.geometry.PointCloud(
            points=o3d.utility.Vector3dVector(mesh_rec.vertices))
    ...
    completion_ratio_rec = completion_ratio(gt_pc_tri.vertices,
                                            rec_pc_tri.vertices)
...
def calc_3d_metric_New(rec_meshfile,
    ...
    path_to_pred_ply = rec_meshfile[:last_slash_index]

    result_3d = run_evaluation(pred_ply,
                               path_to_pred_ply,
...
def calc_2d_metric(rec_meshfile,
                   gt_meshfile,
                   transform_matrix=None,
                   align=True,
                   n_imgs=1000):
    ...
    if transform_matrix is not None:
        ...
        # use icp
        o3d_rec_pc = o3d.geometry.PointCloud(points=rec_mesh.vertices)
        o3d_gt_pc = o3d.geometry.PointCloud(points=gt_mesh.vertices)
        trans_init = np.eye(4)
        threshold = 0.1
        reg_p2p = o3d.pipelines.registration.registration_icp(
            o3d_rec_pc, o3d_gt_pc, threshold, trans_init,
    ...
    for i in range(n_imgs):
        while True:
            # sample view, and check if unseen region is not inside the camera
            # view, if inside, then needs to resample
            up = [0, 0, -1]
            origin = trimesh.sample.volume_rectangular(extents,
                                                       1,
                                                       transform=transform)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-2d',
                        '--metric_2d',
                        action='store_true',
                        help='enable 2D metric')
    parser.add_argument('-3d',
                        '--metric_3d',
                        action='store_true',
                        help='enable 3D metric')
