...
def test_exp_log(Group, device='cuda'):
    """check Log(Exp(x)) == x."""
    a = .2 * torch.randn(2, 3, 4, 5, 6, 7, Group.manifold_dim,
...
def test_inv(Group, device='cuda'):
    ...
    X = Group.exp(
        .1 *
        torch.randn(2, 3, 4, 5, Group.manifold_dim, device=device).double())
    a = (X * X.inv()).log()
...
def test_exp_log_grad(Group, device='cuda', tol=1e-8):
    ...
    a = torch.zeros(1, Group.manifold_dim, requires_grad=True,
                    device=device).double()
    analytical, reentrant, correct_grad_sizes, correct_grad_types = \
        get_analytical_jacobian((a,), fn(a))

    assert torch.allclose(analytical[0],
                          torch.eye(D, device=device).double(),
                          atol=tol)

    a = .2 * torch.randn(
        1, Group.manifold_dim, requires_grad=True, device=device).double()
    analytical, reentrant, correct_grad_sizes, correct_grad_types = \
        get_analytical_jacobian((a,), fn(a))
...
def test_adj_grad(Group, device='cuda'):
    D = Group.manifold_dim
    X = Group.exp(.5 *
                  torch.randn(1, Group.manifold_dim, device=device).double())
